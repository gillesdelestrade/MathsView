...
(function () {
  ...
  /* ===================================================================== */
  /* Les figures interactives : compas, règle, aimantation                 */
  /* ===================================================================== */
  /*
   * `donnees` décrit la figure de départ :
   *   points : [{ nom, p }]         les points nommés, aimantés
   *   traits : [[p, q], …]          les segments dessinés (et aimantants)
   * La fonction rend un `figure(board, ctx)` prêt pour le moteur.
   */
  function fabriqueFigure(donnees) {
    return function (board, ctx) {
      ...
      // Les points nommés, puis toutes les intersections que les tracés font
      // apparaître : c'est là que le crayon doit pouvoir se poser.
      var cercles = [];                 // { c:[x,y], r }
      var aimants = donnees.points.map(function (x) { return x.p; });
      var marques = [];                 // les petits points d'intersection

      function ajouteAimant(p) {
        if (!isFinite(p[0]) || !isFinite(p[1])) return;
        // hors du cadre : inutile de le marquer
      ...
      // Les intersections d'un cercle avec un segment.
      function interCercleSegment(C, s) {
        var u = sub(s[1], s[0]), f = sub(s[0], C.c);
        var a = dot(u, u), b = 2 * dot(f, u), c = dot(f, f) - C.r * C.r;
        var disc = b * b - 4 * a * c;
        if (disc < 0 || a < 1e-12) return [];
        var rd = Math.sqrt(disc), out = [];
        [( -b - rd) / (2 * a), (-b + rd) / (2 * a)].forEach(function (t) {
          if (t >= -0.02 && t <= 1.02) out.push(add(s[0], mul(u, t)));
        });
        return out;
      }
      function nouveauCercle(C) {
        cercles.forEach(function (D) { interCercles(C, D).forEach(ajouteAimant); });
        (donnees.traits || []).forEach(function (t) {
          interCercleSegment(C, t).forEach(ajouteAimant);
        });
        cercles.push(C);
      }
      function aimante(p) {
      ...
      var traces = [];                  // tout ce qu'on peut effacer
      var outil = 'compas';
      var depart = null;
      // L'aperçu : un cercle et un segment, créés une fois pour toutes.
      ...
      // `e.touches`, que `board.getUsrCoordsOfMouse` ne sait pas lire. On passe
      // donc par JXG.getPosition, qui gère la souris ET le doigt.
      function souris(e) {
      ...
      board.on('move', function (e) {
        if (!depart) return;
        var p = outil === 'regle' ? aimante(souris(e)) : souris(e);
        tirer(p);
        var r = dist(depart, p);
        apTexte.setText(outil === 'compas'
          ? O.fr(Math.round(r * 10) / 10) + ' cm'
          : O.fr(Math.round(r * 10) / 10) + ' cm');
      ...
      barre.innerHTML =
        '<button type="button" class="exo-outil actif" data-o="compas">◯ Compas</button>' +
        '<button type="button" class="exo-outil" data-o="regle">📏 Règle</button>' +
        '<button type="button" class="exo-outil gomme">↺ Effacer mes tracés</button>' +
        '<span class="exo-outil-aide">Appuie et tire : le compas trace un cercle, la règle ' +
        'une droite. Le crayon s\'aimante aux points et aux croisements.</span>';
      ...
      Array.prototype.forEach.call(btns, function (b) {
        b.onclick = function () {
          if (b.classList.contains('gomme')) {
            ...
            aimants = donnees.points.map(function (x) { return x.p; });
            board.update();
            return;
          }
          outil = b.dataset.o;
