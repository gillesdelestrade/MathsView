/*
 * Inéquations produit (2nde) — résoudre (ax + b)(cx + d) ⩾ 0 avec un tableau
 * de signes.
 *
 * On ne sait pas « isoler x » dans un produit ; on sait autre chose, qui
 * suffit : le signe d'un produit se lit sur le signe de ses facteurs. La leçon
 * ne fait donc qu'une chose, mais la fait voir :
 *
 *   1) chaque facteur ax + b est une droite ; elle s'annule en x = −b/a, et de
 *      part et d'autre de ce zéro elle est d'un seul signe — celui de a à
 *      droite, le contraire à gauche. Sur la figure, la droite se trace et sa
 *      bande de signe se remplit sous le repère, alignée sur son zéro ;
 *   2) la bande du produit se remplit colonne par colonne avec la règle des
 *      signes : + × − = −, − × − = + ;
 *   3) on garde les colonnes du signe demandé, zéros compris ou non selon que
 *      l'inégalité est large ou stricte, et on lit S sur l'axe.
 *
 * Les trois bandes sous le repère SONT le tableau de signes, aligné sur les
 * zéros des droites au-dessus : c'est la raison d'être de la figure. Le
 * panneau, lui, écrit le tableau comme l'élève doit le faire sur sa copie,
 * ligne par ligne, et les deux disent la même chose.
 *
 * Le point sur l'axe se déplace à la souris et refait le calcul : le signe de
 * chaque facteur, puis celui du produit (vert : l'inéquation est vraie).
 *
 * Le cas du zéro double — (1 − x)(2x − 2) ⩾ 0, produit −2(x − 1)² — est
 * traité, pas écarté : une seule colonne de zéro, un seul signe de chaque
 * côté, et S se réduit à {1}. C'est le piège classique, il mérite sa place.
 *
 * Les étapes règlent un état ABSOLU pour que « Précédent » rejoue la même
 * image ; le rafraîchissement est appelé explicitement, jamais depuis
 * l'événement update ; et tout ce que les textes de la figure lisent est
 * calculé AVANT le premier board.create (JSXGraph évalue un texte dès sa
 * création).
 */
MathsView.register({
  id: 'inequations-produit',
  ...
  description:
    ...
    'se lit sur le signe de ses facteurs — et le signe d\'un facteur \\(ax+b\\) est simple : ' +
    'il s\'annule en \\(x=-\\frac{b}{a}\\), il a le <strong>signe de \\(a\\)</strong> à droite ' +
    'de ce zéro, le signe contraire à gauche.' +
    '<br>La figure trace chaque facteur, remplit sa <strong>bande de signe</strong> sous le ' +
    'repère, alignée sur son zéro, puis la bande du <strong>produit</strong> colonne par colonne ' +
    'avec la règle des signes. Il ne reste qu\'à garder les colonnes du signe demandé.' +
  ...
  notes:
    '<ul>' +
    '<li><strong>Le signe de \\(ax+b\\).</strong> Il s\'annule en \\(x_0=-\\dfrac{b}{a}\\). ' +
    'À droite de \\(x_0\\), \\(ax+b\\) a le signe de \\(a\\) ; à gauche, le signe contraire. ' +
    'Ainsi \\(2x-4\\) est négatif avant \\(2\\), positif après ; \\(-x+3\\) est positif ' +
    'avant \\(3\\), négatif après.</li>' +
    '<li><strong>Le tableau de signes.</strong> Une ligne pour \\(x\\), une par facteur, une ' +
    'pour le produit. On place les zéros dans l\'ordre croissant, on remplit chaque ligne de ' +
    'facteur, puis la ligne du produit avec la <strong>règle des signes</strong> : ' +
    '\\(+\\times+=+\\), \\(+\\times-=-\\), \\(-\\times-=+\\). Le produit est nul là où ' +
    'l\'un des facteurs est nul.</li>' +
    '<li><strong>Lire la réponse.</strong> \\((2x-4)(-x+3)\\geqslant 0\\) : on garde les ' +
    'colonnes où le produit est \\(+\\), et les zéros puisque l\'inégalité est large : ' +
    '\\(S=[2\\,;3]\\). Avec \\(>0\\), les zéros sont exclus : \\(S=\\;]2\\,;3[\\). Avec ' +
    '\\(\\leqslant 0\\), on garde les autres colonnes : ' +
    '\\(S=\\;]-\\infty\\,;2]\\cup[3\\,;+\\infty[\\) — deux morceaux, et le symbole ' +
    '\\(\\cup\\) entre eux.</li>' +
    '<li><strong>Pourquoi pas « comme une équation » ?</strong> Un produit nul a un facteur ' +
    'nul, c\'est vrai. Mais un produit <em>positif</em> n\'a pas « un facteur positif » : ' +
    '\\((-2)\\times(-3)=6\\). Il faut les deux signes à la fois — d\'où le tableau.</li>' +
    '<li><strong>Le zéro double.</strong> \\((1-x)(2x-2)=-2(x-1)^2\\) : les deux facteurs ' +
    's\'annulent en \\(1\\). Le tableau n\'a qu\'une colonne de zéro, le produit est ' +
    'négatif des deux côtés, et \\((1-x)(2x-2)\\geqslant 0\\) n\'a qu\'une solution, ' +
    '\\(S=\\{1\\}\\).</li>' +
    '<li><strong>Vérifier.</strong> On prend un nombre dans une colonne et on calcule les ' +
    'deux facteurs : pour \\(x=2{,}5\\), \\(2x-4=1>0\\) et \\(-x+3=0{,}5>0\\), le produit ' +
    'est positif — c\'est bien ce que dit la colonne.</li>' +
    '</ul>',
  board: {
    boundingbox: [-6.5, 9.6, 6.5, -11.6], keepaspectratio: false,
  ...
  fiche: {
    ...
    figures: [{
      legende: '(2x − 4)(−x + 3) ⩾ 0 : le tableau de signes, et S = [2 ; 3].',
      ...
      dessine: function (board) {
        ...
        // La grille du tableau : 4 lignes, une colonne d'en-tête et 3 colonnes.
        ...
        t(1.3, 3.7, 'x'); t(1.3, 2.7, '2x − 4', b1); t(1.3, 1.7, '−x + 3', b2); t(1.3, 0.7, 'produit');
        t(3.0, 3.7, '−∞'); t(X[2], 3.7, '2'); t(X[3], 3.7, '3'); t(9.6, 3.7, '+∞');
        // les signes, colonne par colonne
        var rows = [[2.7, ['−', '0', '+', '|', '+']], [1.7, ['+', '|', '+', '0', '−']], [0.7, ['−', '0', '+', '0', '−']]];
        rows.forEach(function (r) {
          var y = r[0], s = r[1];
          t(3.6, y, s[0], s[0] === '+' ? ok : no, 12); t(5.6, y, s[2], s[2] === '+' ? ok : no, 12); t(7.6, y, s[4], s[4] === '+' ? ok : no, 12);
          [[X[2], s[1]], [X[3], s[3]]].forEach(function (z) {
            if (z[1] === '0') t(z[0], y, '0', g, 11);
            else board.create('segment', [[z[0], y - 0.45], [z[0], y + 0.45]], { strokeColor: g, strokeWidth: 1, fixed: true, highlight: false });
          });
        });
        // la colonne gardée
    ...
    points: [
      'Un facteur ax + b s\'annule en \\( x_0 = -\\tfrac{b}{a} \\). À <b>droite</b> de \\( x_0 \\) il a le <b>signe de a</b>, à gauche le signe contraire.',
      '<b>Tableau de signes</b> : une ligne x avec les zéros dans l\'ordre, une ligne par facteur, une ligne produit.',
      'Ligne produit : la <b>règle des signes</b> colonne par colonne (+ × − = −, − × − = +) ; 0 sous chaque zéro.',
      'On garde les colonnes du signe demandé. Inégalité <b>large</b> (⩾, ⩽) : les zéros sont solutions ; <b>stricte</b> : non.',
      'Deux morceaux se relient par ∪ : \\( ]-\\infty\\,;2] \\cup [3\\,;+\\infty[ \\).',
      'Vérifier avec un nombre de la colonne : pour x = 2,5, \\( 2x - 4 = 1 \\) et \\( -x + 3 = 0{,}5 \\), produit positif.'
    ],
    exemples: [
      '\\( (2x - 4)(-x + 3) \\geqslant 0 \\) : zéros 2 et 3 ; produit − 0 + 0 − ; \\( S = [2\\,;3] \\). Avec \\( \\leqslant 0 \\) : \\( S = \\;]-\\infty\\,;2] \\cup [3\\,;+\\infty[ \\).',
      '\\( (x + 1)(x - 3) < 0 \\) : zéros −1 et 3 ; produit + 0 − 0 + ; \\( S = \\;]-1\\,;3[ \\) (stricte : zéros exclus).',
      '\\( (1 - x)(2x - 2) \\geqslant 0 \\) : zéro double 1, produit − 0 − ; \\( S = \\{1\\} \\).'
    ]
  },
  ...
  setup: function (board, mv) {
    ...
    function calcule() {
      ...
      for (var i = 0; i + 1 < bornes.length; i++) {
        ...
        cols.push({ lo: lo, hi: hi, mid: mid, s1: sgn(f1(mid)), s2: sgn(f2(mid)), sp: sgn(prod(mid)) });
      }
      var voulu = (rel === 'gt' || rel === 'ge') ? 1 : -1;
      var large = rel === 'ge' || rel === 'le';
      // Les morceaux de S : les colonnes du bon signe, puis les zéros si
      // l'inégalité est large — rattachés à un voisin gardé, ou seuls.
    ...
    function bornesFenetre() {
      ...
      while (XR - XL < 8) { XL--; XR++; }
      board.setBoundingBox([XL - 0.6, 9.6, XR + 0.6, -11.6], false);
    }
    calcule();
    bornesFenetre();
    ...
    function zero(zf, key, col, dy) {
      ...
      var V = board.create('segment', [pt(zf, function () { return 0; }), pt(zf, function () { return ROWS.prod - HB - 0.25; })],
        { strokeColor: col, strokeWidth: 1.2, dash: 2, fixed: true, highlight: false, layer: 2, visible: false });
    ...
    function bande(key) {
      ...
      for (var j = 0; j < 2; j++) {
        (function (j) {
          ...
          zeros.push(board.create('text', [zx, y, function () {
            ...
            var nul = key === 'prod' || (key === 'f1' ? Math.abs(z - D.z1) < 1e-9 : Math.abs(z - D.z2) < 1e-9);
            return nul ? '0' : '|';
      ...
      var nom = etiquette(function () { return XL - 0.45; }, y,
        function () { return key === 'f1' ? facteur(a, b) : key === 'f2' ? facteur(c, d) : 'produit'; },
    ...
    function titreTxt() { return '(' + facteur(a, b) + ')(' + facteur(c, d) + ') ' + RELH[rel] + ' 0'; }
    board.create('text', [cx, 9.0, titreTxt], { anchorX: 'middle', anchorY: 'middle', fontSize: 21, color: INK, cssStyle: 'font-weight:800', fixed: true, highlight: false, layer: 9 });
    function regle(k, col) {
      return '<b style="color:' + col + '">' + facteur(k === 1 ? a : c, k === 1 ? b : d) + '</b> s\'annule en x = ' +
             (k === 1 ? D.z1Txt : D.z2Txt) + ' ; à droite, il a le signe de ' + (k === 1 ? 'a = ' + fmt(a) : 'c = ' + fmt(c)) +
             ' (<b>' + ((k === 1 ? a : c) > 0 ? '+' : '−') + '</b>), à gauche le contraire.';
    }
    function capHaut() {
      if (vis.sol > 0) {
        return 'On garde les colonnes où le produit est <b style="color:' + couleur(D.voulu) + '">' + signeTxt(D.voulu) + '</b>' +
               (D.large ? ', <b>zéros compris</b> (inégalité large)' : ', <b>zéros exclus</b> (inégalité stricte)') + '.';
      }
      if (vis.prod > 0) return 'Le <b style="color:' + C_P + '">produit</b> : la règle des signes, colonne par colonne — + × − = −, − × − = +.';
      if (vis.f2 > 0) return regle(2, C2);
      if (vis.f1 > 0) return regle(1, C1);
      return 'Le signe d\'un produit se lit sur le signe de ses facteurs : on les étudie l\'un après l\'autre.';
    }
    function capBas() {
      if (vis.sol >= 1) return '<b style="color:' + C_OK + '">S = ' + D.Stxt + '</b> — déplace le point pour vérifier le signe en une valeur.';
      if (vis.prod > 0 && D.zs.length === 1) return 'Un seul zéro pour les deux facteurs : le tableau n\'a qu\'une colonne de zéro.';
      return '';
    }
    board.create('text', [cx, 7.9, capHaut], { anchorX: 'middle', anchorY: 'middle', fontSize: 13, color: INK, fixed: true, highlight: false, layer: 9 });
    ...
    var PRESETS = [
      ...
      { label: '(x + 1)(x − 3) < 0', set: function () { a = 1; b = 1; c = 1; d = -3; rel = 'lt'; } },
    ...
    function ligneTable(key, vue, nb) {
      ...
      D.cols.forEach(function (co, i) {
        ...
        if (i < D.zs.length) {
          var z = D.zs[i];
          var nul = key === 'prod' || (key === 'f1' ? Math.abs(z - D.z1) < 1e-9 : Math.abs(z - D.z2) < 1e-9);
          var gardeZ = key === 'prod' && vis.sol > 0 && D.large;
          h += (nb != null && i >= nb) ? '<td class="sg-bar"></td>'
             : '<td class="sg-bar' + (gardeZ ? ' is-sol' : '') + '">' + (nul ? '0' : '<span class="sg-trait"></span>') + '</td>';
        }
      });
      return h + '<td></td></tr>';
    }
    function renderTable() {
      ...
      D.cols.forEach(function (co, i) {
        h += '<td></td>';
        if (i < D.zs.length) h += '<td class="sg-bar">' + D.zTxt[i] + '</td>';
      });
      h += '<td>+∞</td></tr>';
    ...
    function renderNote() {
      if (vis.sol >= 0.999) {
        noteEl.innerHTML = 'Le produit est <b style="color:' + couleur(D.voulu) + '">' + signeTxt(D.voulu) + '</b> dans ' +
          (D.cols.filter(function (co) { return co.sp === D.voulu; }).length || 'aucune') + ' colonne' +
          (D.cols.filter(function (co) { return co.sp === D.voulu; }).length > 1 ? 's' : '') +
          (D.large ? ', et nul en ' + D.zTxt.join(' et ') + ' : les zéros sont solutions' : ' ; les zéros sont exclus') +
          '. <b style="color:' + C_OK + '">S = ' + D.Stxt + '</b>';
      } else if (vis.prod > 0) {
        noteEl.innerHTML = 'Règle des signes : + × + = +, + × − = −, − × − = +. Le produit est nul sous chaque zéro.';
    ...
    function renderPanel() {
      ...
      panel.innerHTML =
        ...
        '<p style="margin:.2rem 0 0">x = <strong>' + fmt(xv) + '</strong> : ' +
          ...
          ' (' + signeTxt(sgn(v2)) + ') ; produit ' + fmt(vp) + ' → <strong style="color:' + (ok(xv) ? C_OK : C_NO) + '">' +
    ...
    function clampInputs() {
      ...
      if (a === 0) a = 1;                                 // un facteur sans x n'est plus un facteur
      if (c === 0) c = 1;
