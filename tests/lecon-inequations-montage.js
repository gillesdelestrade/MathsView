/*
 * La leçon « Inéquations du premier degré » (2nde), montée avec le VRAI JSXGraph.
 *
...
load('lessons/2nde/inequations-premier-degre.js');
...
var pts = board.objectsList.filter(function (o) { return o.elType === 'point' && o.getAttribute('fixed') === false; });
...
if (!err.length) {
  ...
  rands[1].onclick(); jouer('dé avec fraction');
  try {
    pts[0].setPosition(JXG.COORDS_BY_USER, [1.5, 0]);
